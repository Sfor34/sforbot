...
import os, requests, json, time, re, sqlite3, random, hashlib
...
#              ⚙️ تنظیمات اصلی
# ═══════════════════════════════════════════
TOKEN      = os.environ.get('TOKEN', '8576320592:AAFLipMaJiDFJyDCalDemS5ZikTcM-OYC-0')
...
║   ✅ Gemini 2.5 Flash AI            ║
║   ✅ Anti-Link/Spam/Profanity       ║
║   ✅ Full Group Management          ║
║   ✅ Config & VPN Distribution      ║
║   ✅ Glass Panel UI                 ║
║   ✅ Fun Features (Jokes...)        ║
║   ✅ File Upload via Bot            ║
╚══════════════════════════════════════╝
    ''')
    time.sleep(3)  # صبر کن Flask کامل بیاد بالا
    while True:
        try:
...
if __name__ == '__main__':
    Thread(target=start_polling, daemon=False).start()
    # Flask در thread جداست (خط بالا) - اینجا فقط polling رو نگه میداریم
    # main thread باید زنده بمونه
    while True:
        time.sleep(60)
