...
if not TOKEN:
    print('[SFOR] ❌ TOKEN env var is missing! Set TOKEN in Render environment.', flush=True)
    sys.exit(1)
...
ALLOWED_GROUP_FIELDS = {
    'anti_link', 'anti_spam', 'anti_profanity', 'welcome', 'goodbye',
    'ai_reply', 'warn_limit', 'rules', 'welcome_msg', 'goodbye_msg', 'filter_words'
}
...
_auto_replies_cache = None
_auto_replies_cache_time = 0

def get_auto_replies(force_refresh=False):
    global _auto_replies_cache, _auto_replies_cache_time
    if force_refresh or _auto_replies_cache is None or (time.time() - _auto_replies_cache_time) > 60:
        conn = db()
        c = conn.cursor()
        c.execute('SELECT keyword,response,match_type FROM auto_replies')
        rows = c.fetchall()
        conn.close()
        _auto_replies_cache = [(r[0], r[1], r[2]) for r in rows]
        _auto_replies_cache_time = time.time()
    return _auto_replies_cache
...
_x = b'Z3NrX2VmVDJIWU4xUlNOOUxtUzRvZ0t5V0dkeWIzRll0OFFSWWhQMnh2eDVwUENuRWVIaFprRG4='
GROQ_KEY = os.environ.get('GROQ_KEY') or _b64.b64decode(_x).decode()
GROQ_URL = 'https://api.groq.com/openai/v1/chat/completions'
GROQ_MODEL = 'llama-3.3-70b-versatile'

SYSTEM_PROMPT = f"""تو دستیار هوشمند ربات SFOR هستی با اسم «اسفور» یا «SFOR AI».

🔥 درباره SFOR:
- SFOR یه پلتفرم فناوری و امنیت سایبری ایرانیه
- سایت: {SITE_URL}
- اینستاگرام: @mmadsfor
- ربات تلگرام: @sfor_bot
- تیم: mmad sfor (مدیر) + علی (دکتر علی، @Ali_Crypto00) + شهاب شعبانی (امنیت)
- نسخه ربات: {BOT_VERSION}

✅ خدمات SFOR:
- آموزش امنیت سایبری و هک اخلاقی
- ابزارهای امنیتی و فناوری
- فیلترشکن رایگان (V2Ray و VPN) — دستور /config یا /vpn
- هوش مصنوعی فارسی رایگان
- مدیریت هوشمند گروه تلگرام
...
- /ai سوال — هوش مصنوعی
- /weather شهر — آب‌وهوا
- /price — قیمت ارز و کریپتو

قوانین پاسخ:
- همیشه فارسی جواب بده (مگر کاربر انگلیسی بپرسه)
- کوتاه و مفید باش (حداکثر ۱۵۰ کلمه)
- دوستانه، صمیمی و باانرژی باش
- وقتی درباره SFOR یا ربات پرسیدن، با افتخار معرفی کن
- اگه درباره فیلترشکن پرسیدن: /config یا /vpn رو معرفی کن
- اگه سوال خطرناک/غیراخلاقی بود: ادبانه رد کن
- اگه کاربر ناراحته: همدردی کن و کمک کن
- در پایان پاسخ‌های مهم، سایت SFOR رو معرفی کن"""
...
def ask_gemini(uid, message):
    ...
    try:
        ...
        add_ai_history(uid, 'assistant', reply)
        return reply
...
#         📋 راهنما
# ═══════════════════════════════════════════
HELP_TEXT = """📖 *راهنمای ربات SFOR v3*
...
/price — قیمت ارز و کریپتو
/ip آدرس — اطلاعات IP
/short لینک — کوتاه‌کننده
/translate متن — ترجمه به فارسی
...
/remind دقیقه متن — یادآور
/timer ثانیه — تایمر
/birthday — تولد مبارک

💬 *کلمات کلیدی گروه (بدون /):*
`جوک` — جوک تصادفی
`رنکینگ` — رنک گروه
`قیمت` — نرخ ارز
`فیلترشکن` — راهنمای VPN
`سایت` — لینک SFOR
`انگیزه` — نقل‌قول
`بازی` — شروع بازی
`هوش مصنوعی` — راهنمای AI
`پنل` — پنل ادمین (فقط ادمین)

🛡️ *مدیریت گروه (ادمین):*
/warn دلیل — اخطار (روی پیام reply کن)
/unwarn — رفع اخطار
/warns — تعداد اخطارها
...
/note نام متن — ذخیره یادداشت

🔥 *ادمین اصلی:*
/panel — پنل مدیریت کامل
/broadcast متن — پیام همگانی
/addconfig نام لینک — افزودن کانفیگ
/addvpn — آپلود فایل VPN
/addreply کلمه | پاسخ — پاسخ خودکار
/logs — لاگ رویدادها
...
def persistent_keyboard():
    ...
    m.add(KeyboardButton('📋 منو'), KeyboardButton('🤖 هوش مصنوعی'))
    m.add(KeyboardButton('🔒 کانفیگ'), KeyboardButton('📁 VPN'))
    m.add(KeyboardButton('📚 آموزش'), KeyboardButton('🆔 آیدی من'))
    return m
...
def start_cmd(message):
    ...
    if is_admin:
        text = (f'🔥 خوش اومدی *{name}* — ادمین اصلی!\n\n'
                f'🛠️ پنل مدیریت در دسترسه.\n'
                f'برای پنل ادمین: /panel\n'
                f'برای راهنما: /help')
    else:
    ...
    if not is_admin:
        bot.send_message(message.chat.id, '⬇️ منوی کامل:', reply_markup=main_menu())
...
@bot.message_handler(func=lambda m: m.text in ['📋 منو', '🤖 هوش مصنوعی', '🔒 کانفیگ', '📁 VPN', '📚 آموزش', '🆔 آیدی من'])
def keyboard_buttons(message):
    add_user(message.from_user)
    t = message.text
    if t == '📋 منو':
        bot.send_message(message.chat.id, '📋 *منوی اصلی:*', parse_mode='Markdown', reply_markup=main_menu())
    elif t == '🤖 هوش مصنوعی':
        bot.send_message(message.chat.id, '🤖 سوالت رو بنویس:\n`/ai سوالت`', parse_mode='Markdown')
    elif t == '🔒 کانفیگ':
        config_cmd(message)
    elif t == '📁 VPN':
        vpn_cmd(message)
    elif t == '📚 آموزش':
        bot.send_message(message.chat.id, f'📚 آموزش‌های SFOR:\n{SITE_URL}/tutorials')
    elif t == '🆔 آیدی من':
        u = message.from_user
        bot.send_message(message.chat.id, f'👤 *{u.first_name}*\n🆔 `{u.id}`\n📛 @{u.username or "ندارد"}', parse_mode='Markdown')
...
# Rate limit برای AI (5 ثانیه بین هر درخواست)
_ai_rate_limit = {}
...
def ai_cmd(message):
    ...
    # بررسی rate limit
    now_ts = time.time()
    last_req = _ai_rate_limit.get(uid, 0)
    if now_ts - last_req < 5:
        bot.reply_to(message, '⏳ کمی صبر کن! (هر ۵ ثانیه یه درخواست)')
        return
    _ai_rate_limit[uid] = now_ts
    msg = bot.reply_to(message, '🤖 *در حال پردازش...*', parse_mode='Markdown')
    reply = ask_gemini(uid, parts[1])
    try:
...
def del_cmd(message):
    ...
    if not is_group_admin(message.chat.id, message.from_user.id):
        bot.reply_to(message, '⛔ فقط ادمین‌ها.'); return
    if not message.reply_to_message:
        bot.reply_to(message, '↩️ روی پیامی که میخوای حذف بشه Reply کن.'); return
    try:
...
# ─── آمار گروه ───
@bot.message_handler(commands=['stat', 'stats', 'آمار'])
def stat_cmd(message):
    ...
    conn = db()
    c = conn.cursor()
    c.execute('SELECT COUNT(*) FROM warns WHERE group_id=?', (message.chat.id,))
    warns_count = c.fetchone()[0]
    conn.close()
    al = g['anti_link'] if g else 0
    asp = g['anti_spam'] if g else 0
    apr = g.get('anti_profanity', 0) if g else 0
    bot.reply_to(message,
        f'📊 *آمار گروه {message.chat.title}*\n\n'
        f'👥 اعضا: `{count}` نفر\n'
        f'⚠️ اخطارهای فعال: `{warns_count}`\n'
        f'🔗 ضد لینک: {"✅" if al else "❌"}\n'
        f'🚫 ضد اسپم: {"✅" if asp else "❌"}\n'
        f'🤬 ضد فحش: {"✅" if apr else "❌"}\n'
        f'⚠️ حد اخطار: `{g["warn_limit"] if g else 3}`',
        parse_mode='Markdown')

# ─── اطلاعات کاربر ───
@bot.message_handler(commands=['info', 'اطلاعات'])
def info_cmd(message):
    ...
    conn = db()
    c = conn.cursor()
    c.execute('SELECT message_count, is_banned, is_vip FROM users WHERE id=?', (u.id,))
    row = c.fetchone()
    conn.close()
    mc = row[0] if row else 0
    banned = '🔨 بله' if row and row[1] else '✅ خیر'
    vip = '⭐ بله' if row and row[2] else '❌ خیر'
    warns = get_warns(u.id, message.chat.id) if message.chat.type != 'private' else 0
    bot.reply_to(message,
        f'👤 *اطلاعات کاربر*\n\n'
        f'📛 نام: {u.first_name}\n'
        f'🆔 آیدی: `{u.id}`\n'
...
# ─── سکوت همه (فقط ادمین‌ها پیام بدن) ───
@bot.message_handler(commands=['slowmode', 'ro', 'فقط_ادمین'])
def ro_cmd(message):
    if message.chat.type == 'private': return
    if not is_group_admin(message.chat.id, message.from_user.id):
        bot.reply_to(message, '⛔ فقط ادمین‌ها.'); return
    ...
    if len(parts) > 1 and parts[1] == 'off':
        try:
            bot.set_chat_permissions(message.chat.id,
                ChatPermissions(can_send_messages=True, can_send_media_messages=True,
                                can_send_polls=True, can_send_other_messages=True,
                                can_add_web_page_previews=True))
            bot.reply_to(message, '🔓 گروه باز شد — همه میتونن پیام بدن.')
        except Exception as e:
            bot.reply_to(message, f'❌ {e}')
    else:
        try:
            ...
            bot.reply_to(message, '🔒 گروه قفل شد — فقط ادمین‌ها میتونن پیام بدن.\nبرای باز کردن: /ro off')
        except Exception as e:
            bot.reply_to(message, f'❌ {e}')

# ─── منشن همه اعضا (ادمین) ───
@bot.message_handler(commands=['all', 'همه', 'tagall'])
def tagall_cmd(message):
    if message.chat.type == 'private': return
    if not is_group_admin(message.chat.id, message.from_user.id):
        bot.reply_to(message, '⛔ فقط ادمین‌ها.'); return
    parts = message.text.split(maxsplit=1)
    text = parts[1] if len(parts) > 1 else '📢 توجه!'
    conn = db()
    c = conn.cursor()
    c.execute('SELECT id, first_name FROM users WHERE is_banned=0 LIMIT 50')
    rows = c.fetchall()
    conn.close()
    if not rows:
        bot.reply_to(message, '❌ کاربری پیدا نشد.'); return
    mentions = ' '.join([f'[{r[1]}](tg://user?id={r[0]})' for r in rows])
...
@bot.message_handler(commands=['clearai', 'پاک_ai'])
def clearai_cmd(message):
    clear_ai_history(message.from_user.id)
    bot.reply_to(message, '🗑️ تاریخچه AI پاک شد. مکالمه جدید شروع میشه!')

@bot.message_handler(commands=['about', 'درباره'])
def about_cmd(message):
    bot.reply_to(message,
        f'ℹ️ *ربات SFOR v{BOT_VERSION}*\n\n'
        '🤖 نام: اسفور (SFOR AI)\n'
        f'🌐 سایت: {SITE_URL}\n'
        '📸 اینستاگرام: @mmadsfor\n'
        '👨‍💻 تیم: mmad sfor + دکتر علی\n\n'
        '🛡️ امنیت | هک | فناوری | هوش مصنوعی\n\n'
...
def welcome_handler(message):
    ...
    for member in message.new_chat_members:
        ...
        # ─── کپچا (اگر ماژول features لود شده) ───
        feat = loaded_modules.get('sfor_features')
        if feat and g:
            try:
                feat.send_captcha(message.chat.id, member.id, name)
                continue  # کپچا جایگزین خوش‌آمد میشه
            except Exception:
                pass
        # ─── خوش‌آمد معمولی ───
        if not g or not g['welcome']: continue
...
def group_message_handler(message):
    ...
    if message.text and message.text.strip() in ['پنل', 'panel']:
        if is_group_admin(message.chat.id, uid):
            ...
            bot.reply_to(message, '⛔ فقط ادمین‌ها میتونن پنل رو ببینن.')
        return
    ...
    if message.text:
        ...
        if t in ['جوک', 'joke', 'بگو جوک']:
            bot.reply_to(message, random.choice(JOKES)); return

        if t in ['خلوته', 'خلوت', 'کسی نیست', 'کسی هست']:
            bot.reply_to(message, random.choice(LONELY_REPLIES)); return

        if t in ['رنک', 'رنکینگ', 'rank', 'برترین‌ها', 'بهترین‌ها']:
            feat = loaded_modules.get('sfor_features')
            if feat:
                bot.reply_to(message, feat.get_ranking_text(cid), parse_mode='Markdown')
            return

        if t in ['امتیاز', 'score', 'امتیاز من']:
            feat = loaded_modules.get('sfor_features')
            if feat:
                sc = feat.get_user_score(uid, cid)
                bot.reply_to(message, f'⭐ امتیاز تو: `{sc}`', parse_mode='Markdown')
            return

        if t in ['قوانین', 'rules', 'قانون']:
            g2 = get_group(cid)
            rules = g2['rules'] if g2 and g2['rules'] else 'قوانینی تنظیم نشده.'
            bot.reply_to(message, f'📌 *قوانین گروه:*\n\n{rules}', parse_mode='Markdown')
            return

        if t in ['قیمت', 'price', 'بیت کوین', 'btc', 'کریپتو']:
            feat = loaded_modules.get('sfor_features')
            if feat:
                msg2 = bot.reply_to(message, '⏳ در حال دریافت...')
                result = feat.get_prices()
                from telebot.types import InlineKeyboardMarkup, InlineKeyboardButton
                m2 = InlineKeyboardMarkup()
                m2.add(InlineKeyboardButton('🔄 بروز', callback_data='refresh_price'))
                bot.edit_message_text(result, cid, msg2.message_id, parse_mode='Markdown', reply_markup=m2)
            return

        if t in ['انگیزه', 'نقل قول', 'quote']:
            feat = loaded_modules.get('sfor_features')
            if feat:
                bot.reply_to(message, random.choice(feat.QUOTES), parse_mode='Markdown')
            return

        if t in ['بازی', 'game', 'بازی کن']:
            feat = loaded_modules.get('sfor_features')
            if feat:
                result = feat.start_number_game(cid)
                bot.reply_to(message, result, parse_mode='Markdown')
            return

        if t in ['سایت', 'وبسایت', 'لینک سایت']:
            bot.reply_to(message, f'🌐 سایت SFOR:\n{SITE_URL}'); return

        if t in ['فیلترشکن', 'vpn', 'کانفیگ', 'config']:
            bot.reply_to(message, '🔒 برای دریافت فیلترشکن رایگان:\n/config — کانفیگ V2Ray\n/vpn — فایل VPN'); return

        if t in ['هوش مصنوعی', 'ai', 'ربات هوشمند', 'اسفور']:
            bot.reply_to(message, '🤖 برای استفاده از AI بنویس:\n`/ai سوالت`\n\nیا منو رو مستقیم منشن کن!', parse_mode='Markdown'); return

    # ─── AI گروه (منشن یا ریپلای) ───
    if g and g['ai_reply'] and message.text:
        bot_uname = get_bot_username()
        is_mention = f'@{bot_uname}' in message.text
        is_reply_to_bot = (
            message.reply_to_message
            and message.reply_to_message.from_user
            and message.reply_to_message.from_user.is_bot
            and message.reply_to_message.from_user.username
            and message.reply_to_message.from_user.username.lower() == bot_uname.lower()
        )
        if is_mention or is_reply_to_bot:
            question = message.text.replace(f'@{bot_uname}', '').strip()
            if question:
    ...
    # ─── امتیاز و بازی حدس عدد (از sfor_features) ───
    feat = loaded_modules.get('sfor_features')
    if feat and message.text:
        try:
            feat.add_score(uid, cid, 1)
            result = feat.check_number_game(cid, message.text)
            if result:
                bot.reply_to(message, result, parse_mode='Markdown')
        except Exception:
            pass
...
def private_message_handler(message):
    ...
    # دستورات با / رو skip کن (هندلرهای جداگانه دارن)
    if text.startswith('/'): return

    # ─── پنل ادمین (فقط برای ادمین اصلی) ───
    if message.from_user.id == ADMIN_ID:
        if text in ['پنل', 'panel', 'ادمین']:
            stats = get_stats()
            t = (f'🔥 *پنل ادمین SFOR v{BOT_VERSION}*\n\n'
                 f'👥 کاربران: `{stats["users"]}`\n'
                 f'🏘️ گروه‌ها: `{stats["groups"]}`\n'
                 f'🔒 کانفیگ باقیمانده: `{stats["configs"]}`\n'
                 f'📁 VPN باقیمانده: `{stats["vpns"]}`\n'
                 f'💬 پاسخ خودکار: `{stats["replies"]}`\n'
                 f'📋 لاگ: `{stats["logs"]}`')
            bot.send_message(message.chat.id, t, parse_mode='Markdown', reply_markup=admin_menu())
            return

    # جوک
    if any(w in text.lower() for w in ['جوک', 'joke']):
        bot.reply_to(message, random.choice(JOKES)); return

    # AI در خصوصی
    msg = bot.reply_to(message, '🤖 در حال پردازش...')
    reply = ask_gemini(message.from_user.id, text)
    try:
        bot.edit_message_text(f'🤖 *SFOR AI:*\n\n{reply}',
...
#         🔘 Callbacks
# ═══════════════════════════════════════════
@bot.callback_query_handler(func=lambda c: not (
    c.data.startswith('adv_') or c.data.startswith('captcha_') or c.data == 'refresh_price'
))
...
def load_modules():
    ...
    for f in files:
        ...
        try:
            ...
            mod.is_group_admin = is_group_admin
            mod.update_group = update_group
            mod.safe_delete = safe_delete
            mod.safe_send   = safe_send
            mod.add_warn    = add_warn
            mod.get_warns   = get_warns
            mod.clear_warns = clear_warns
            mod.remove_warn = remove_warn
            mod.ban_user    = ban_user
            mod.unban_user  = unban_user
            mod.mute_user   = mute_user
            mod.ask_gemini  = ask_gemini
            spec.loader.exec_module(mod)
            if hasattr(mod, 'setup'):
                mod.setup(bot, {
                    'ADMIN_ID': ADMIN_ID, 'SITE_URL': SITE_URL,
                    'DB_FILE': DB_FILE, 'db': db,
                    'add_log': add_log, 'add_user': add_user,
                    'add_group': add_group, 'get_group': get_group,
                    'update_group': update_group,
                    'is_group_admin': is_group_admin,
                    'safe_delete': safe_delete, 'safe_send': safe_send,
                    'add_warn': add_warn, 'get_warns': get_warns,
                    'clear_warns': clear_warns, 'remove_warn': remove_warn,
                    'ban_user': ban_user, 'unban_user': unban_user,
                    'mute_user': mute_user, 'ask_gemini': ask_gemini,
                })
            print(f'[SFOR] ✅ module: {name}', flush=True)
            loaded_modules[name] = mod
        except Exception as e:
            print(f'[SFOR] ❌ module {name}: {e}', flush=True)
