...
#         🤖 Groq AI
# ═══════════════════════════════════════════
GROQ_KEY = os.environ.get('GROQ_KEY', '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')
...
🌐 *ابزارها:*
/weather شهر | /هوا — آب‌وهوا
/price | /قیمت — قیمت کریپتو
/ip آدرس — اطلاعات IP
/short لینک | /کوتاه — کوتاه‌کننده لینک
/translate متن | /ترجمه — ترجمه به فارسی
/calc عبارت | /حساب — ماشین حساب
/remind دقیقه متن | /یادآور — یادآور
/timer ثانیه | /تایمر — تایمر

🎮 *سرگرمی:*
/game | /بازی — بازی حدس عدد
/rank | /رنکینگ — رنکینگ گروه
/score | /امتیاز — امتیاز من
/quote | /انگیزه — نقل‌قول
جوک — جوک خنده‌دار
خلوته — من اینجام!
...
#         💬 پیام‌های خصوصی
# ═══════════════════════════════════════════
@bot.message_handler(func=lambda m: m.chat.type == 'private' and not (
    m.from_user.id == ADMIN_ID and
    m.from_user.id in getattr(loaded_modules.get('sfor_advanced'), 'pending_v2ray', {})
))
