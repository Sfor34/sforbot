# ═══════════════════════════════════════════
#     🔥 SFOR Features Module v2.1
#     فایل: modules/sfor_features.py
# ═══════════════════════════════════════════
import time, re, requests, random, sqlite3, threading
...
def setup(bot_instance, ctx):
    ...
    global add_warn, get_warns, clear_warns, remove_warn, ban_user, mute_user, ask_gemini
    bot = bot_instance
    ...
    remove_warn    = ctx.get('remove_warn', lambda u,g: None)
...
def get_weather(city):
    try:
        ...
        desc_list = current.get('lang_fa') or current.get('weatherDesc', [{}])
        ...
        return f'❌ شهر پیدا نشد. مثال: `/weather Tehran`'
...
def get_prices():
    try:
        ...
        return '❌ خطا در دریافت قیمت‌ها. لطفاً دوباره امتحان کن.'
...
def send_captcha(cid, uid, name):
    ...
    options = list(set([max(1, o) for o in [
        answer, answer+random.randint(1,5), answer-random.randint(1,4),
        answer+random.randint(6,10), answer-random.randint(5,9), random.randint(1,20)
    ]]))[:6]
...
def register_handlers():
    ...
    def weather_cmd(message):
        ...
        if len(parts) < 2:
            bot.reply_to(message, '🌤️ مثال: `/weather Tehran`', parse_mode='Markdown')
            return
    ...
    def short_cmd(message):
        parts = message.text.split(maxsplit=1)
        if len(parts) < 2:
            bot.reply_to(message, '🔗 مثال: `/short https://example.com`', parse_mode='Markdown')
            return
        url = parts[1].strip()
        if not url.startswith('http'):
            bot.reply_to(message, '❌ لینک باید با http شروع بشه.')
            return
    ...
    def ip_cmd(message):
        parts = message.text.split(maxsplit=1)
        if len(parts) < 2:
            bot.reply_to(message, '🌐 مثال: `/ip 8.8.8.8`', parse_mode='Markdown')
            return
    ...
    def rank_cmd(message):
        if message.chat.type == 'private':
            bot.reply_to(message, '⚠️ این دستور فقط در گروه کار میکنه.')
            return
    ...
    def remind_cmd(message):
        ...
        if len(parts) < 3 or not parts[1].isdigit():
            bot.reply_to(message, '🔔 مثال: `/remind 30 خرید کردن`\n(۳۰ دقیقه دیگه یادآوری میکنه)', parse_mode='Markdown')
            return
        minutes = int(parts[1])
        if minutes > 1440:
            bot.reply_to(message, '⚠️ حداکثر ۱۴۴۰ دقیقه (یک روز)')
            return
        text = parts[2]
    ...
    def timer_cmd(message):
        ...
        if len(parts) < 2 or not parts[1].isdigit():
            bot.reply_to(message, '⏱️ مثال: `/timer 60` (ثانیه)', parse_mode='Markdown')
            return
    ...
    def translate_cmd(message):
        parts = message.text.split(maxsplit=1)
        if len(parts) < 2:
            bot.reply_to(message, '🌐 مثال: `/translate Hello World`', parse_mode='Markdown')
            return
        # rate limit
        uid = message.from_user.id
        now = time.time()
        last = _ai_rate.get(uid, 0)
        if now - last < 5:
            bot.reply_to(message, '⚠️ کمی صبر کن!')
            return
        _ai_rate[uid] = now
        text = parts[1]
    ...
    def font_cmd(message):
        parts = message.text.split(maxsplit=1)
        if len(parts) < 2:
            bot.reply_to(message, '✨ مثال: `/font متن شما`', parse_mode='Markdown')
            return
        text = parts[1]
        results = (
            f'✨ *فونت‌های زیبا:*\n\n'
            f'1: 𝗦𝗙𝗢𝗥 ← Bold\n'
            f'2: 𝘚𝘍𝘖𝘙 ← Italic\n'
            f'3: 𝙎𝙁𝙊𝙍 ← Bold Italic\n\n'
            f'متن اصلی: *{text}*'
        )
        bot.reply_to(message, results, parse_mode='Markdown')

    # ─── ماشین حساب ───
    @bot.message_handler(commands=['calc', 'حساب'])
    def calc_cmd(message):
        parts = message.text.split(maxsplit=1)
        if len(parts) < 2:
            bot.reply_to(message, '🔢 مثال: `/calc 2+2*5`', parse_mode='Markdown')
            return
        expr = parts[1].strip()
        # فقط عملیات ریاضی ساده مجاز
        if not re.match(r'^[\d\s\+\-\*\/\(\)\.]+$', expr):
            bot.reply_to(message, '❌ فقط اعداد و عملگرهای ریاضی مجازند.')
            return
        try:
            result = eval(expr)
    ...
    # ─── تولد مبارک ───
    @bot.message_handler(commands=['birthday', 'تولد'])
    def birthday_cmd(message):
        if not message.reply_to_message:
            bot.reply_to(message, '↩️ روی پیام کسی Reply کن.')
            return
        target = message.reply_to_message.from_user
        msgs = [
            f'🎂 *تولدت مبارک {target.first_name}* عزیز! 🎉\n\n🎁 امیدوارم همه آرزوهات برآورده بشه!\n🌟 سالت پر از موفقیت باشه!',
            f'🎈 *{target.first_name}* جان، تولدت مبارک! 🥳\n\n✨ همیشه شاد و سالم باشی!\n🎊 سال نوتو تبریک میگم!',
        ]
        bot.reply_to(message, random.choice(msgs), parse_mode='Markdown')
    ...
    def captcha_callback(call):
        ...
        if uid != target_uid:
            bot.answer_callback_query(call.id, '⛔ این کپچا برای تو نیست!', show_alert=True)
            return
...
print('[SFOR] ✅ sfor_features module imported!', flush=True)
